...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            return [[prev]] + subseq_helper(s[1:], s[0])
        else:
            b = subseq_helper(s[1:], s[0])
            return [[prev]] + insert_into_all(prev, b) + b
    if (not s):
        return [[]]
...
class Keyboard:
    ...
    def typing(self, typing_input):
        ...
        output = ''
        for cur in typing_input:
            output += self.press(cur)
        return output
...
def trade(first, second):
    ...
    equal_prefix = lambda: bool(sum(first[:m]) == sum(second[:n]))
...
def shuffle(cards):
    ...
    for i in range(half):
        ...
        shuffled = shuffled + [cards[i + half]]
    return shuffled
