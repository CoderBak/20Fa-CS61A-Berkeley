...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        copy_of_player_deck = player.deck.cards[:]
        for current_card in copy_of_player_deck:
            current_card.attack += other_card.attack
            current_card.defense += other_card.defense
        copy_of_opponent_deck = opponent.deck.cards[:]
        cnt = -1
        for opponent_card in copy_of_opponent_deck:
            cnt += 1
            if(opponent_card.attack == other_card.attack and opponent_card.defense == other_card.defense):
                del opponent.deck.cards[cnt]
                cnt -= 1
