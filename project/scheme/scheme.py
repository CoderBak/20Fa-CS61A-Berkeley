...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        return None
    # if there is nothing last, it is a tail call
    current_val = scheme_eval(expressions.first, env, expressions.rest is nil)
    next_val = eval_all(expressions.rest, env)
    if next_val == None:
        return current_val
    return next_val
    # END PROBLEM 7
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        formal_lst = pair2lst(formals)
        val_lst = pair2lst(vals)
        for i in range(len(formal_lst)):
            new_frame.define(formal_lst[i], val_lst[i])
        return new_frame
        # END PROBLEM 10
...
def pair2lst(pr):
    if pr == nil:
        return []
    return [pr.first] + pair2lst(pr.rest)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        function_args = target.rest
        args_lst = pair2lst(function_args)
        for args in args_lst:
            if not scheme_symbolp(args):
                raise SchemeError('invalid args')
        function_body = expressions.rest
        env.define(function_name, LambdaProcedure(function_args, function_body, env))
...
# remember that the last part of and and or is tail call
...
def do_and_form(expressions, env):
    ...
    for i in range(length - 1):
        cur_expr = expr[i]
        current = scheme_eval(cur_expr, env)
        if is_false_primitive(current):
            return False
    return scheme_eval(expr[length - 1], env, True)
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    value_lst = []
    # BEGIN PROBLEM 14
    bind_lst = pair2lst(bindings)
    for binding in bind_lst:
        validate_form(binding, 2, 2)
        current_name = binding.first
        current_val = scheme_eval(binding.rest.first, env)
        if not scheme_symbolp(current_name):
            raise SchemeError('bad arguments')
        if current_name in name_lst:
            raise SchemeError('bind several times')
        name_lst.append(current_name)
        value_lst.append(current_val)
    names = lst2pair(name_lst)
    values = lst2pair(value_lst)
    # END PROBLEM 14
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        for args in args_lst:
            if not scheme_symbolp(args):
                raise SchemeError('invalid args')
        function_body = expressions.rest
        env.define(function_name, MacroProcedure(function_args, function_body, env))
        return function_name
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
...
class Thunk(object):
    ...
    primarily used to delay a calculation until its result is needed,
