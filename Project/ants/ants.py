...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if(isinstance(self, ContainerAnt) == isinstance(place.ant,ContainerAnt)):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while(current_place != beehive):
            current_bees = rANTdom_else_none(current_place.bees)
            if(current_bees != None and self.min_range <= cnt <= self.max_range):
                return current_bees
            else:
    ...
    def action(self, gamestate):
        ...
        if(not self.is_true):
            ...
            while(current_place != None):
                cur_ant = current_place.ant
                if(cur_ant != None and (not cur_ant.buffed)):
                    cur_ant.damage *= 2
                    cur_ant.buffed = True
...
def apply_status(status, bee, length):
    ...
    # BEGIN Problem Optional 4
    original_action = bee.action
    changed_action = status(bee.action, bee)
    def updated_action(gamestate):
        nonlocal length
        if(length > 0):
            new_action = changed_action
            length -= 1
        else:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while(True):
            if(current_place == None):
                break
            if(current_place == beehive):
                break
            if(current_place.ant != None and current_place.ant != self):
                dict_of_insect[current_place.ant] = cnt
            bee_lst = current_place.bees[:]
            for bee in bee_lst:
                dict_of_insect[bee] = cnt
            current_place = current_place.entrance
            cnt += 1
        return dict_of_insect
        # END Problem Optional 5
    ...
    def action(self, gamestate):
        ...
        for insect, distance in insects_and_distances.items():
            ...
            Insect.reduce_armor(insect, damage)
            if damage:
                self.insects_shot += 1
